...
export async function GET(request, { params }) {
    ...
    try {
        ...
        if (!ALLOWED_SLUGS.includes(slug)) {
            return apiError('Invalid admissions slug', { status: 404 });
        }
        ...
        return handleApiError(error, 'Error fetching admissions admin content:', 'Failed to fetch admissions admin content');
    }
}
    ...
    try {
        ...
        if (id) {
            ...
            return apiSuccess(updated[0], {
                message: 'Admissions content updated successfully',
            });
        }
        ...
        return apiSuccess(created[0], {
            status: 201,
            message: 'Admissions content created successfully',
        });
    } catch (error) {
        return handleApiError(error, 'Error saving admissions content:', 'Failed to save admissions content');
    }
}
...
export async function DELETE(request, { params }) {
    ...
    try {
        ...
        return apiSuccess({}, {
            message: 'Admissions content deleted successfully',
        });
    } catch (error) {
        return handleApiError(error, 'Error deleting admissions content:', 'Failed to delete admissions content');
    }
}
