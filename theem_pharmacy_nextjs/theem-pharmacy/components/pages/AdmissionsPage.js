        ...
        <main className="bg-[#f8fafc] min-h-screen pb-12">
            ...
            <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-[-60px] relative z-20">
                <div className="grid lg:grid-cols-2 gap-12">
                    {[
                        ...
                        <article key={i} className="bg-white rounded-2xl shadow-2xl shadow-gray-200/60 border-t-8 overflow-hidden group hover:translate-y-[-8px] transition-all duration-500" style={{ borderTopColor: program.color }}>
                            <div className="p-6 lg:p-8">
                                <div className="flex items-start justify-between mb-6">
                                    <div className="space-y-1">
                                        <div className="text-xs font-bold uppercase tracking-wider opacity-50">Program Overview</div>
                                        <h3 className="text-2xl lg:text-3xl font-black text-[var(--brand-primary)] tracking-tight">{program.name}</h3>
                                        <div className="text-[var(--bcp-teal)] font-black text-xs uppercase tracking-widest">{program.code} ({program.duration})</div>
                                    </div>
                                    <div className="w-12 h-12 rounded-xl bg-[var(--brand-primary-soft)] flex items-center justify-center text-[var(--brand-primary)] group-hover:bg-[var(--brand-primary)] group-hover:text-white transition-all">
                                        <i className={`fas ${i === 0 ? 'fa-user-md' : 'fa-pills'} text-xl`}></i>
                                    </div>
                                </div>
                ...
                <div className="bg-[var(--brand-primary)] rounded-2xl p-8 lg:p-14 text-white relative overflow-hidden">
                    <div className="text-center mb-10 relative z-10">
                        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[var(--brand-primary)]/5 border border-[var(--brand-primary)]/10 mb-2">
                                    ...
                                    <span className="text-xs font-bold uppercase tracking-wider text-[var(--brand-primary)]">Step-by-Step</span>
                                </div>
                        <h2 className="text-2xl font-black tracking-tight">Admission Protocol</h2>
                    </div>
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 relative z-10">
                        {[
                            ...
                            <div key={i} className="space-y-3 text-center">
                                <div className="w-12 h-12 bg-white/10 rounded-xl flex items-center justify-center text-white mx-auto mb-4 group-hover:scale-110 transition-transform">
            ...
            <section className="py-14 max-w-5xl mx-auto px-4">
                <div className="text-center mb-10">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[var(--brand-primary)]/5 border border-[var(--brand-primary)]/10 mb-2">
                                    <span className="w-1.5 h-1.5 rounded-full bg-[var(--brand-accent)]"></span>
                                    <span className="text-xs font-bold uppercase tracking-wider text-[var(--brand-primary)]">Calendar</span>
                                </div>
                    <h2 className="text-2xl font-black text-[var(--brand-primary)] tracking-tight">Important Dates</h2>
                </div>
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {[
                        ...
                        <div key={i} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-xl shadow-gray-200/40 flex flex-col items-center text-center group hover:border-[var(--bcp-teal)] transition-all">
                            <div className={`w-10 h-10 rounded-lg bg-${item.color}-50 text-${item.color}-600 flex items-center justify-center mb-3 transition-transform group-hover:rotate-12`}>
                                <i className={`fas ${item.icon} text-sm`}></i>
                            </div>
                            <h4 className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1">{item.event}</h4>
                            <div className="text-lg font-black text-[var(--brand-primary)]">{item.date}</div>
                        </div>
                    ))}
                </div>
            </section>
